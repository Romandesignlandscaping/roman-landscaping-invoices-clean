...
const STORAGE_KEY = 'roman-landscaping-clean-v1';
...
function defaultState() {
  ...
  return {
    ...
    settings: {
      ...
      promoImage: '/assets/promo-default-v2.png',
      headerLogo: '/assets/header-logo-v2.png',
      defaultDueDays: 14,
      compactView: false,
...
async function warmImages() {
  const paths = ['/assets/header-logo-v2.png', '/assets/promo-default-v2.png'];
  ...
  return `
    ...
    <section class="grid-2">
      ...
      <div class="section-card">
        ...
        <div class="preview-box"><img id="promoPreview" src="${state.settings.promoImage || '/assets/promo-default-v2.png'}" alt="Promo preview"></div>
...
function createInvoiceSnapshot(invoice) {
  ...
  return {
    ...
    promoImage: state.settings.promoImage || '/assets/promo-default-v2.png',
    headerLogo: state.settings.headerLogo || '/assets/header-logo-v2.png',
...
async function generateInvoicePdf(invoice, { download = true, doc = null, addPage = false } = {}) {
  ...
  const headerLogoPath = snap.headerLogo || state.settings.headerLogo || '/assets/header-logo-v2.png';
  const headerLogo = await loadImageAsDataUrl(headerLogoPath);
  const promoPath = snap.promoImage || state.settings.promoImage || '/assets/promo-default-v2.png';
  ...
  let promoWidth = Math.min(contentWidth - 120, 440);
  ...
  if (promoHeight < 140) {
    localDoc.addPage();
    y = 40;
    promoWidth = Math.min(contentWidth - 120, 440);
    promoHeight = promoWidth / promoRatio;
  }
  localDoc.addImage(promo, 'PNG', pageWidth / 2 - promoWidth / 2, y + 8, promoWidth, promoHeight, undefined, 'FAST');
